...
/*
 1. Here is no macro for classes in a namespace and one for objects
 can not use constants. Let's define one.
*/
...
ZEND_BEGIN_ARG_INFO(ArgInfo_sample_test, 0)
	...
	*/
	SAMPLE_ARG_OBJ_INFO_NS(0, e, PHP_SAMPLE_EXT_NS, SAMPLE_CLASS_NAME, 0)
ZEND_END_ARG_INFO()
...
PHP_FUNCTION(sample_test)
{
	...
	php_printf("Argument is an %s\\%s.\n", PHP_SAMPLE_EXT_NS, SAMPLE_CLASS_NAME);
}

const zend_function_entry php_sample_functions[] = {
	ZEND_NS_NAMED_FE(PHP_SAMPLE_EXT_NS, test, ZEND_FN(sample_test), ArgInfo_sample_test)
	PHP_FE_END
};
...
PHP_MINIT_FUNCTION(sample)
{
	zend_class_entry ce;
	INIT_NS_CLASS_ENTRY(
		ce, PHP_SAMPLE_EXT_NS, SAMPLE_CLASS_NAME, php_sample_class_functions
	);
